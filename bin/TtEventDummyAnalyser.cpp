...
int main() {
  ...
  //
  TFile fIn("../test/TQAFLayer2Output.root");
  TTree * events = (TTree *) fIn.Get( "Events" );
  assert( events != 0 );
  TBranch * decayB = events->GetBranch( "recoCandidatesOwned_decaySubset__TQAF.obj" );
  TBranch * genEvtB = events->GetBranch( "TtGenEvent_genEvt__TQAF.obj" );
  TBranch * solsBranch = events->GetBranch( "TtSemiEvtSolutions_solutions__TQAF.obj" );
  assert( solsBranch != 0 );
  ...
  for( int ev = 0; ev < nev; ++ev ) { 
    ...
    if (sols.size()>0) {
     if (sols[0].getGenEvent().isNull() ||
         (//sols[0].getGenEvent()->isSemiLeptonic() &&
          sols[0].getGenEvent()->numberOfBQuarks() == 2)) {

      //get bestSol
      int bestSol = sols[0].getMCBestJetComb();

      if (bestSol < 0 || bestSol >= (int) sols.size()) continue;
